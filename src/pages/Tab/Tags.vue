      ...
      <div class="col-8 q-pa-lg">
        <div class="q-mb-md">
          <div class="row justify-end q-py-md">
            <q-btn color="primary" text-color="white" :label="`New Tag`" @click="isCreateDialogOpened = true" />
          </div>
        </div>

        <q-table :data="tags" :columns="columns" row-key="title" />
      </div>
    </div>
    ...
    <q-dialog v-model="isCreateDialogOpened" position="bottom">
      <q-card style="width: 500px">
        ...
        <q-card-section class="row q-gutter-sm justify-center">
          <div class="col-12">
            <q-input filled v-model="form.title" label="Name" />
          </div>
        </q-card-section>
...
export default {
  ...
  methods: {
    ...
    async onAdd() {
      try {
        await this.$store.dispatch("AddTag", this.form);
