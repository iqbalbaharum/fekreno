<template>
  <q-card style="min-width: 100%">
          ...
          <div>
            <q-item-label 
              class="text-weight-bold text-capitalize">
                {{ journal.category }}
                <q-badge v-if="journal.status === 'new'" color="positive"> {{ journal.status }} </q-badge>
                <q-badge v-if="journal.status === 'review'" color="warning"> {{ journal.status }} </q-badge>
                <q-badge v-if="journal.status === 'discuss'" color="blue"> {{ journal.status }} </q-badge>
            </q-item-label>
            <q-item-label class="text-capitalize text-caption">{{ date.formatDate(journal.createdAt, 'DD MMM YYYY HH:mm A') }}</q-item-label>
        </div>
        <div class="justify-between justify-center">
          <q-btn flat dense size="sm" icon="edit" label="edit" @click="onClickEdit">
    ...
    <q-card-section v-html="journal.detail">
      <q-card-section class="bg-yellow-1" v-if="editdialog.show">
        <q-editor 
          v-model="form.detail"
          min-height="6rem"
          :toolbar="[]"
        ...
        <q-btn :loading="progress" class="flex items-end q-mt-sm" color="primary" text-color="white" label="Edit Journal" @click="onClickConfirm('new')">
