import {
  ...
  setToken,
  removeToken
} from './../../datasources/localstorage.storage';
...
const user = {
  ...
  actions: {
    ...
    CreateNewPassword({ commit }, data) {
      return new Promise((resolve, reject) => {
        this.$repository.user
          .createNewPwd(data.token, data.password)
          .then(res => {
            console.log(res.data);
            resolve(res.data);
          })
    ...
    async GetUserJournal({ commit, rootState }) {
      return new Promise((resolve, reject) => {
        ...
        this.$repository.user
          .getUserJournal(rootState.user.userId, filter)
          .then(res => {
            // clear table
            Journal.deleteAll();
            // insert
    ...
    async GetUserRepositories({ commit, rootState }) {
      return new Promise((resolve, reject) => {
        ...
        this.$repository.user
          .getUserRepositories(rootState.user.userId, filter)
          .then(res => {
            // insert
    ...
    async GetUserProfile({ commit, rootState }) {
      return new Promise((resolve, reject) => {
        this.$repository.user
          .getUserProfile(rootState.user.userId)
          .then(res => {
    ...
    async AssignUserRole({ commit }, data) {
      let res = await this.$repository.user.assignUserRoles(
        data.userId,
        data.roleId
      );
      return res.data;
    ...
    async UnassignUserRole({ commit }, data) {
      let res = await this.$repository.user.unassignUserRoles(
        data.userId,
        data.roleId
      );
      return res.data;
    ...
    ApplyUserApplication({ commit }, data) {
      return new Promise((resolve, reject) => {
        this.$repository.user
          .applyUserApplication(data.applicationId)
          .then(res => {
            UserApplication.insert({ data: res.data });
            resolve(res.data);
          })
    ...
    async UnapplyUserApplication({ commit }, data) {
      let res = await this.$repository.user.unapplyUserApplication(
        data.applicationId
      );
      return res.data;
    },
    ...
    async UpdateUserProfile({ commit, rootState }, data) {
      this.$repository.user
        .updateUserProfile(rootState.user.userId, data)
        .then(res => {
